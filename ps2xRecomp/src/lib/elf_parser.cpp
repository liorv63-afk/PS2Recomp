...
namespace ps2recomp
{
    ...
    std::vector<Function> ElfParser::extractFunctions() const
    {
        ...
        // A Ghidra-provided end is authoritative even when the name is auto-generated
        // (FUN_/sub_/etc.) -- which is virtually every function in a stripped retail
        // binary. Without this exemption, Ghidra's correct, explicit boundary gets
        // silently discarded and the function falls back to "extend to the next
        // authoritative-or-symbol function start" -- which, when nothing else is
        // registered for a long stretch afterward, can swallow the rest of the
        // executable section into one function and try to decode trailing data as code.
        for (const auto &extra : m_extraFunctions)
        {
            ...
            if (IsAutoGeneratedName(extra.name) &&
    ...
    void ElfParser::setSkipJalScanFallback(bool skip)
    {
        m_skipJalScanFallback = skip;
    }
    ...
    void ElfParser::loadDebugFunctions()
    {
        ...
        if (m_extraFunctions.empty() && !m_skipJalScanFallback)
        {
